...
export default function Ventas2() {
  ...
  const getData = async () => {
    try {
      const response = await axios.get('https://deploybackendtp-44411f5799d1.herokuapp.com/reporteVentas/ventas2');
  ...
  const labels = data.map((dato) => dato.cliente);
  const values = data.map((dato) => dato.saldoTotal);
