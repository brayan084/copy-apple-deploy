...
export default function Clientes() {
    ...
    const getProductos = async () => {
        ...
        axios.get('https://deploybackendtp-44411f5799d1.herokuapp.com/clientes').then(r => {
            setProducts(r.data)
        }).catch(error => {
            console.log(error);
        });
    ...
    const saveProduct = async (e) => {
        ...
        try {
            if (product.id) {
                await axios.put(`https://deploybackendtp-44411f5799d1.herokuapp.com/clientes/${product.id}`, product);
                const updatedProducts = [...products];
                const productIndex = updatedProducts.findIndex((p) => p.id === product.id);
                updatedProducts[productIndex] = { ...product };
                setProducts(updatedProducts);
                toast.current.show({ severity: 'success', summary: 'Exitoso', detail: 'Cliente editado', life: 3000 });
            } else {
                const response = await axios.post('https://deploybackendtp-44411f5799d1.herokuapp.com/clientes/createClientes', product);
    ...
    const deleteProduct = async (e) => {
        try {
            if (product.id) {
                await axios.delete(`https://deploybackendtp-44411f5799d1.herokuapp.com/clientes/${product.id}`);
